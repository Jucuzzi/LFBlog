...
-(void)setUpUI{
    ...
    self.leftV.backgroundColor = [UIColor blackColor];
    self.leftV.layer.shadowColor = [UIColor blackColor].CGColor;//shadowColor阴影颜色
    self.leftV.layer.shadowOffset = CGSizeMake(0,3);//shadowOffset阴影偏移,x向右偏移2，y向下偏移6，默认(0, -3),这个跟shadowRadius配合使用
    self.leftV.layer.shadowOpacity = 0.2;//阴影透明度，默认0
    self.leftV.layer.shadowRadius = 3;//阴影半径，默认3
    self.leftV.layer.cornerRadius = 10;
    [self.mainScrollView addSubview:self.leftV];
    ...
    self.centerV.backgroundColor = [UIColor blackColor];
    self.centerV.layer.shadowColor = [UIColor blackColor].CGColor;//shadowColor阴影颜色
    self.centerV.layer.shadowOffset = CGSizeMake(0,3);//shadowOffset阴影偏移,x向右偏移2，y向下偏移6，默认(0, -3),这个跟shadowRadius配合使用
    self.centerV.layer.shadowOpacity = 0.2;//阴影透明度，默认0
    self.centerV.layer.shadowRadius = 3;//阴影半径，默认3
    self.centerV.layer.cornerRadius = 10;
    [self.mainScrollView addSubview:self.centerV];
    ...
    self.rightV.backgroundColor = [UIColor blackColor];
    self.rightV.layer.shadowColor = [UIColor blackColor].CGColor;//shadowColor阴影颜色
    self.rightV.layer.shadowOffset = CGSizeMake(0,3);//shadowOffset阴影偏移,x向右偏移2，y向下偏移6，默认(0, -3),这个跟shadowRadius配合使用
    self.rightV.layer.shadowOpacity = 0.2;//阴影透明度，默认0
    self.rightV.layer.shadowRadius = 3;//阴影半径，默认3
    self.rightV.layer.cornerRadius = 10;
    [self.mainScrollView addSubview:self.rightV];
...
-(void)updateViewFrameSetting{
    ...
    self.rightV.frame = self.rightIV.frame;
...
- (void)scrollViewDidScroll:(UIScrollView *)scrollView {
    if (self.itemMargnPadding > 0) {
        ...
        if (currentX > 0) { //左滑
            ...
            self.centerV.height = ZXMainScrollViewHeight - variableH;
            self.centerIV.y = currentX/ZXMainScrollViewWidth*self.imageHeightPoor;
            self.centerV.y = currentX/ZXMainScrollViewWidth*self.imageHeightPoor;
            self.rightIV.height = ZXMainScrollViewHeight-2*self.imageHeightPoor+variableH;
            self.rightV.height = ZXMainScrollViewHeight-2*self.imageHeightPoor+variableH;
            self.rightIV.y = self.imageHeightPoor-currentX/ZXMainScrollViewWidth*self.imageHeightPoor;
            self.rightV.y = self.imageHeightPoor-currentX/ZXMainScrollViewWidth*self.imageHeightPoor;
            ...
            self.leftV.height = ZXMainScrollViewHeight-2*self.imageHeightPoor-variableH;
            self.leftIV.y = self.imageHeightPoor+currentX/ZXMainScrollViewWidth*self.imageHeightPoor;
            self.leftV.y = self.imageHeightPoor+currentX/ZXMainScrollViewWidth*self.imageHeightPoor;
        } else {
            self.leftIV.alpha = self.initAlpha;
            self.centerIV.alpha = 1;
            self.rightIV.alpha = self.initAlpha;
            self.leftIV.height = ZXMainScrollViewHeight-2*self.imageHeightPoor;
            self.leftV.height = ZXMainScrollViewHeight-2*self.imageHeightPoor;
            self.centerIV.height = ZXMainScrollViewHeight;
            self.centerV.height = ZXMainScrollViewHeight;
            self.rightIV.height = ZXMainScrollViewHeight-2*self.imageHeightPoor;
            self.rightV.height = ZXMainScrollViewHeight-2*self.imageHeightPoor;
            self.leftIV.y = self.imageHeightPoor;
            self.leftV.y = self.imageHeightPoor;
            self.centerIV.y = 0;
            self.centerV.y = 0;
            self.rightIV.y = self.imageHeightPoor;
            self.rightV.y = self.imageHeightPoor;
        }
    }
}
@end
